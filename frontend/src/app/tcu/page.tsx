...
import {
  getTcuLeads, getTcuStats, runTcuPipeline, ingestTcuText, ingestTcuPdf,
} from "@/lib/api";
import { formatCurrency, formatDate } from "@/lib/utils";

const ACT_LABELS: Record<string, string> = {
  citacao: "Citação (débito)",
  audiencia: "Audiência",
  notificacao: "Notificação",
  acordao_condenatorio: "Acórdão condenatório",
  edital: "Edital / Pauta",
  outro: "Outro",
};
...
const TEMA_LABELS: Record<string, string> = {
  educacao_fnde: "Educação / FNDE",
  saude: "Saúde",
  assistencia_social: "Assistência Social / FNAS",
  infraestrutura: "Infraestrutura / DNIT",
  cultura_fnc: "Cultura / FNC",
  previdencia: "Previdência / INSS",
...
export default function TcuPage() {
  ...
  const [f, setF] = useState<any>({
    ...
    valor_min: "", only_opportunities: false, hide_represented: false,
    search: "", order_by: "score",
  });
  ...
  const load = useCallback(() => {
    ...
    setLoading(true);
    Promise.all([getTcuLeads(params), getTcuStats()])
      .then(([l, s]) => { setLeads(l.data); setStats(s.data); })
      .catch(() => flash("Erro ao carregar leads."))
  ...
  async function handleIngest() {
    ...
    try {
      let r;
      if (ingestTab === "text") {
        if (!ingestText.trim()) { flash("Cole o texto do caderno."); setIngesting(false); return; }
        r = await ingestTcuText({ text: ingestText, publication_date: ingestDate || undefined });
      } else {
        ...
        r = await ingestTcuPdf(fd);
      }
      const d = r.data;
      flash(d.message ? d.message : `Ingestão concluída: ${d.created} novos, ${d.duplicated} duplicados.`);
      setShowIngest(false);
      setIngestText(""); setIngestFile(null);
      load();
    } catch (e: any) {
      flash(e.response?.data?.detail || "Erro na ingestão.");
  ...
  return (
    <AppShell>
      <Header
        title="Radar TCU"
        subtitle="Monitoramento do Diário Eletrônico/BTCU e das APIs abertas do TCU para identificação de oportunidades"
        actions={
          <div className="flex items-center gap-2">
            <Link href="/tcu/config" className="btn-secondary">Configuração</Link>
      ...
      {/* Banner de conformidade LGPD/OAB */}
      <div className="mb-5 bg-amber-500/5 border border-amber-500/25 rounded-lg px-4 py-3 text-xs text-amber-200/90 flex items-start gap-2">
        ...
        <p>
          <strong>Ferramenta interna de inteligência.</strong> Os dados vêm de fontes públicas (Diário Oficial/BTCU) e servem à
          priorização de trabalho e qualificação de oportunidades. Não há contato automático com as partes intimadas —
          a captação de clientela é vedada (OAB Prov. 205/2021, arts. 3º e 6º). Registre a base de legítimo interesse (LGPD)
          e a objeção do titular em cada lead.
        </p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          ...
          <StatCard label="Débito monitorado" value={formatCurrency(stats.valor_total_debito)} hint="soma dos débitos" />
        </div>
      )}
      ...
      <div className="card mb-5 space-y-3">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          <input className="input" placeholder="Buscar nome/CPF/processo..." value={f.search}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <input className="input max-w-[10rem]" type="number" placeholder="Débito mínimo R$" value={f.valor_min}
            onChange={e => setF({ ...f, valor_min: e.target.value })} />
          <input className="input max-w-[6rem]" placeholder="UF" maxLength={2} value={f.uf}
            onChange={e => setF({ ...f, uf: e.target.value.toUpperCase() })} />
          <label className="flex items-center gap-2 text-sm text-slate-300">
            <input type="checkbox" checked={f.only_opportunities} onChange={e => setF({ ...f, only_opportunities: e.target.checked })} />
            Só oportunidades
          </label>
          <label className="flex items-center gap-2 text-sm text-slate-300">
            <input type="checkbox" checked={f.hide_represented} onChange={e => setF({ ...f, hide_represented: e.target.checked })} />
            Ocultar já representados
          </label>
              ...
              {leads.map(l => (
                <tr key={l.id} className="hover:bg-surface-light/30">
                  ...
                  <td className="table-td">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full border ${ACT_COLORS[l.act_type] || ACT_COLORS.outro}`}>
                      {ACT_LABELS[l.act_type] || l.act_type}
                    </span>
                  ...
                  <td className="table-td">
                    {l.prazo_final ? formatDate(l.prazo_final) : <span className="text-muted">—</span>}
                  </td>
                  <td className="table-td">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${LEAD_STATUS_COLORS[l.status] || "text-muted bg-muted/10"}`}>
                      {LEAD_STATUS_LABELS[l.status] || l.status}
                    </span>
                  </td>
                  <td className="table-td"><Link href={`/tcu/${l.id}`} className="text-primary text-xs hover:underline">Abrir →</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Modal isOpen={showIngest} onClose={() => setShowIngest(false)} title="Ingerir edição do Diário / BTCU">
        <div className="space-y-4">
          <p className="text-xs text-muted">
            Como o endpoint de listagem do BTCU não é documentado, você pode alimentar o sistema manualmente colando o texto de um caderno
            ou enviando o PDF baixado de <code className="text-slate-300">obterDocumentoPdf</code>. Útil também para backfill.
          </p>
