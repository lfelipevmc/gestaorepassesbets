        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card">
            <div className="flex items-start justify-between gap-4 mb-3">
              ...
              {lead.ja_representado && <span className="text-xs px-2.5 py-1 rounded-full bg-muted/10 text-muted border border-surface-border">Parte já representada</span>}
            </div>
          ...
          <div className="card grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Field label="Prazo final">{lead.prazo_final ? <span className="text-warning font-medium">{formatDate(lead.prazo_final)}</span> : null}</Field>
          </div>

          <div className="card">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-white text-sm">Enriquecimento cadastral (CNPJ)</h3>
              {lead.doc_type === "cnpj" && !enr && <button onClick={enrich} disabled={saving} className="btn-secondary text-xs">Enriquecer via BrasilAPI</button>}
            </div>
            {lead.doc_type !== "cnpj" ? (
        ...
        <div className="space-y-6">
          <div className="card">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {lead.assignee_id !== me?.id && <button onClick={() => patch({ assignee_id: me?.id }, "Lead atribuído a você.")} className="btn-secondary text-xs">Assumir</button>}
            </div>
            <label className="label">É oportunidade?</label>
            <div className="flex gap-2">
              <button onClick={() => patch({ is_opportunity: true })} className={`text-xs px-3 py-1.5 rounded-lg border ${lead.is_opportunity ? "bg-success/10 text-success border-success/30" : "border-surface-border text-muted"}`}>Sim</button>
              <button onClick={() => patch({ is_opportunity: false })} className={`text-xs px-3 py-1.5 rounded-lg border ${!lead.is_opportunity ? "bg-danger/10 text-danger border-danger/30" : "border-surface-border text-muted"}`}>Não</button>
            </div>
          </div>
          ...
          <div className="card border-amber-500/20">
            ...
            <label className="flex items-center gap-2 text-sm text-slate-300 mb-3">
              <input type="checkbox" checked={lead.lgpd_objection} onChange={e => patch({ lgpd_objection: e.target.checked }, "Objeção LGPD registrada.")} />
              Titular exerceu direito de oposição (LGPD)
            </label>
            {lead.lgpd_objection && <div className="mb-3 text-xs bg-danger/10 border border-danger/30 text-danger rounded-lg px-3 py-2">Objeção registrada — não utilizar estes dados para prospecção.</div>}
            ...
            <button onClick={() => patch({ legitimate_interest_basis: lgpdBasis }, "Base registrada.")} disabled={saving} className="btn-secondary text-xs mt-2 w-full">Salvar base</button>
          </div>
          ...
          <div className="card">
            ...
            <div className="flex gap-2 mb-3">
              <input className="input" placeholder="Nova anotação..." value={noteText} onChange={e => setNoteText(e.target.value)} onKeyDown={e => { if (e.key === "Enter") submitNote(); }} />
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              ...
              {(lead.notes || []).map((n: any) => (
                <div key={n.id} className="text-xs border-l-2 border-surface-border pl-3">
                  ...
                  <p className="text-muted mt-0.5">{n.kind === "status_change" ? "🔄 " : ""}{formatDateTime(n.created_at)}</p>
                </div>
              ))}
            </div>
          </div>
